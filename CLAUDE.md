...
## Session log — 2026-07-22: per-parcel sales history (ledger + county bulk extract) — shipped live

Answered "how far back can we pull?" and ended up solving the layer's core limitation. The GIS layer holds only the **latest sale per parcel** (oldest surviving Dublin priced sale: 1977), so widening `months_back` yields survivorship remnants, not history. Built two complementary fixes, **shipped live** (commit `79e8504`, CI+Deploy green, live-verified):

- **Append-only ledger** — `franklin_housing/history.py` (zero-dep): `data/sales_history.sqlite`, PK `(parcelid, sale_date, sale_price)`, `INSERT OR IGNORE`, **never cleared by any code path** — the one NON-derivable data file (no VPS backup yet — open item). Both refresh paths append (try/except, never blocks): `server/jobs/refresh.py` (`source=webapp`) + CLI `--refresh` (`source=cli`). `scripts/sales_history.py` `backfill|report`. Backfill recovered 33 already-overwritten sales from the 7/11 baseline snapshot. Live ledger self-seeded via in-container refresh (13,671 sales at `/data/sales_history.sqlite`).
- **THE BIG FIND — county bulk extract**: `apps.franklincountyauditor.com/outside_user_files/<year>/<YYYY-MM-DD> Appraisal/` (monthly, ~15th evening ET, free, no auth). Its `Sales*.txt` (in `Tab-Delimited.zip`, ~77MB) carry **one row per CONVEYANCE back to ~1980** incl. the county's real **`VALID` arms-length coding** — the field that's null in every GIS feed and that we've proxied since day one. Files split by tax-district ranges (`Sales010`/`Sales020-277`/`Sales410-610`); our parcels span districts 273/590/212/201/270 → two files. Parcel ids are site format (`273-003074-00` → GIS `273-003074`, suffix collapsed, `parcelid_raw` kept).
- **`franklin_housing/bulk_sales.py`** (zero-dep): discovers newest extract, downloads bundle (temp-file lifecycle helper), auto-selects needed Sales files by district range, filters to tracked parcels, **atomic idempotent rebuild** of `sales` + `sales_meta` in the same DB as `parcels` (true 1:many join). Result: **63,631 conveyances / 13,672 of 13,678 parcels (99.96%), 12,593 county-coded**. Hygiene flags surfaced not dropped (`bad_date`/`zero_price`/`multi_parcel`/`conditional_sale`). Run: `.venv/bin/python -m franklin_housing.bulk_sales [db]`.
- **VALID upgrade**: `annotate_valid()` matches each parcel's latest GIS sale to its coded bulk row (price equal + date ±10d, abstain-don't-mislabel), maps `0 - VALID`→Y / numeric reason codes→N / non-numeric (`Y - WF FLAG`)→abstain. Applied at **read time** (`ReadRepo.records()`, refresh-job trend materialization, CLI) because the daily refresh rewrites parcels with null VALID. `clean.py` precedence needed zero changes. 5,330 parcels now county-judged (4,429 Y / 901 N); `valid_basis: county_coded+ratio_proxy`. Whigham oracle held ($523,040).
- **API/UI**: `/api/report` carries `sale_history`; `/api/meta` carries `bulk_sales` ingest info; new `SaleHistoryTable.jsx` on the address report (mobile card-reflow pattern).
- **Quality review** (7 findings, all fixed): the notable one — **materialized trends disagreed with live-annotated records** (caught by existing test) → refresh job now annotates before `build_report`; also `db_diff._table()` now explicitly prefers `parcels` (was first-table-by-creation-order, would KeyError on the new `sales` tables), download temp-file leak, ingest `busy_timeout`, `_num` de-dup.
- **VPS**: seeded live ingest (identical numbers), in-container refresh re-materialized trends (pull 55), installed `/etc/cron.d/franklin-bulk-sales` (17th 08:00 UTC, pattern-matched to `franklin-refresh`). Gotchas: app dir needs `sudo bash -c 'cd … && docker compose …'` (cd itself needs root); container snapshot step warns on read-only relative `data/` (pre-existing, by-design continue).
- Tests 27→34, ruff clean, frontend builds. Docs updated: WEBAPP.md (new "Sale history" section + API table), deploy/README.md (monthly cron section).
